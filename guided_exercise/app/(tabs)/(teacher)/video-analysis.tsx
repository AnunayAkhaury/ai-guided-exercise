...
import TEST_VIDEO from '../../../src/assets/images/exercise-test.mp4';
...
export default function VideoAnalysis() {
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.notSupported}>
          Video analysis with QuickPose is only available on iOS devices.
        </Text>
      </View>
    );
  }
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      const videoUri = asset.localUri!;

      const result = await processVideo(videoUri, QUICKPOSE_SDK_KEY, (progress) => {
      ...
      setState({
        ...
        feedbacks:
          result.feedbacks.length > 0
            ? result.feedbacks
            : ['No form corrections detected – great work!'],
      });
    } catch (err: any) {
      ...
      {state.status === 'done' && (
        <View style={styles.resultsBox}>
          ...
          <Text style={styles.resultsSubtitle}>
            QuickPose detected the following corrections across your video:
          </Text>
