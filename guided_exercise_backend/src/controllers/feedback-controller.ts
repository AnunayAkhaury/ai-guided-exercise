...
export async function addClipController(req: Request, res: Response) {
  ...
  const { clipUrl, exercise, userId, duration, starttime } = req.body;
  try {
    const clipId = await addClip(clipUrl, exercise, userId, duration, starttime);
...
export async function addFeedbackController(req: Request, res: Response) {
  ...
  const { clipId, exercise, feedbackJson, starttime, userId } = req.body;
  try {
    const feedback_id = await addFeedback(clipId, exercise, feedbackJson, starttime, userId);
