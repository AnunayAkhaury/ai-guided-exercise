...
async function deleteCollection(collectionName: string, batchSize = 300) {
  while (true) {
...
async function deleteAllAuthUsers() {
  ...
  do {
    ...
    if (uids.length > 0) {
      ...
      console.log(
        `[reset-firebase] Deleted auth users: ${uids.length - deleteResult.failureCount}/${uids.length}`
      );
    }
